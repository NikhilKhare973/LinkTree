...
export default async function Page({ params }) {
    ...
    const item2 = {
        ...
        "desc": "A description is a statement or piece of writing that uses words to explain what someone or something is like, including its characteristics."
    }
            ...
            <div className="photo flex justify-center flex-col items-center gap-4">
                ...
                <div className="links" >
                    {item.links.map((item, index) => {
                        return <Link key={index} target="_black" href={item.link}> <div className="bg-purple-100 hover:bg-purple-500 hover:shadow-lg py-4 shadow-lg px-2 min-w-96 flex justify-center rounded-md my-3">
                            {item.linktext}
                        </div> </Link>
                    })}
                </div>
